...
class Bot:
    def __init__(self, symbol: str, Open: [float], Close: [float], High: [float], Low: [float], Volume: [float], Date: [str], OP: int, CP: int, index: int, tick: float,
        ...
        self.pos = None
        self.avg = None
        self.unusual_vol_down = None
        self.unusual_vol_up = None
    ...
    def update_indicators(self):
        ...
        if self.strategy == 'StochRSIMACD':
            ...
            self.EMA_long = np.array(ema_indicator(pd.Series(self.Close), window=20))
        elif self.strategy == 'JL ATR + Unusual Volume':
            if self.TP_SL_choice == 'Custom':
                self.use_close_pos = True
            self.pos, self.avg, self.unusual_vol_down, self.unusual_vol_up = \
            TS.gen_inputs_for_JL_ATR(self.Date, self.High, self.Low, self.Close, self.Open, self.Volume)
    ...
    def Make_decision(self):
        ...
        if self.strategy == 'StochRSIMACD':
            ...
            if self.TP_SL_choice == 'Custom':
                Trade_Direction, _, stop_loss_val, take_profit_val = TS.JL_ATR_and_Unusual_Volume(Trade_Direction, self.Close,
                                                                  self.pos, self.avg, self.unusual_vol_down,
                                                                  self.unusual_vol_up, self.current_index)
            else:
                Trade_Direction, _, _, _ = TS.JL_ATR_and_Unusual_Volume(Trade_Direction, self.Close,
                                                                        self.pos, self.avg, self.unusual_vol_down,
                                                                        self.unusual_vol_up, self.current_index)
                if Trade_Direction != -99:
                    stop_loss_val, take_profit_val = TS.SetSLTP(self.stop_loss_val, self.take_profit_val, self.peaks,
                                                                ...
                                                                self.SL_mult, self.TP_mult, self.TP_SL_choice,
                                                                self.current_index)
    ...
    def check_close_pos(self, Trade_Direction):
        ...
        if self.strategy == 'heikin_ashi_ema2':
            _, close_pos = TS.heikin_ashi_ema2(self.Open_H, self.High_H, self.Low_H,
                                               self.Close_H, -99,
                                               Trade_Direction, 0, self.fastd, self.fastk,
                                               self.EMA200, self.current_index)
        elif self.strategy == 'heikin_ashi_ema':
            _, close_pos = TS.heikin_ashi_ema(self.Open_H, self.Close_H, -99, Trade_Direction, 0,
                                              self.fastd, self.fastk, self.EMA200, self.current_index)
        elif self.strategy == 'JL ATR + Unusual Volume':
            _, close_pos, _, _ = TS.JL_ATR_and_Unusual_Volume(Trade_Direction, self.Close,
                                                              self.pos, self.avg, self.unusual_vol_down,
                                                              self.unusual_vol_up, self.current_index)
        return close_pos
