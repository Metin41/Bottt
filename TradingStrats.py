...
def nz(y):
    ## Removes NaNs
    return [x if not(math.isnan(x)) else 0 for x in y]
...
def gen_inputs_for_JL_ATR(date, high, low, close, Open, volume):
    atrPeriod = 20
    atrMultiplier = 4
    vwma_length = 20
    vol_length = 20
    ...
    ATR = list(sma_indicator(pd.Series([max(high[i] - low[i], abs(high[i] - close[i]), abs(low[i] - close[i])) for i in range(len(close))]), atrPeriod))
    nLoss = [x * atrMultiplier for x in ATR]

    xATRTrailingStop = []
    for i in range(len(close)):
        if i == 0:
            xATRTrailingStop.append(0)
        elif close[i] > nz2(xATRTrailingStop[i - 1]) and close[i - 1] > nz2(xATRTrailingStop[i - 1]):
            xATRTrailingStop.append(max(nz2(xATRTrailingStop[i-1]), close[i] - nLoss[i]))
        elif close[i] < nz2(xATRTrailingStop[i - 1]) and close[i - 1] < nz2(xATRTrailingStop[i - 1]):
            xATRTrailingStop.append(min(nz2(xATRTrailingStop[i-1]), close[i] + nLoss[i]))
        elif close[i] > nz2(xATRTrailingStop[i-1]):
            xATRTrailingStop.append(close[i] - nLoss[i])
        else:
            xATRTrailingStop.append(close[i] + nLoss[i])
    ...
    for i in range(len(close)):
        if i == 0:
            pos.append(0)
        elif close[i - 1] < nz2(xATRTrailingStop[i - 1]) < close[i]:
            pos.append(1)
        elif close[i - 1] > nz2(xATRTrailingStop[i - 1]) > close[i]:
            pos.append(-1)
        else:
            pos.append(nz2(pos[i-1]))

    vwma = [0 if x == 0 else w / x for w, x in zip(list(sma_indicator(pd.Series([y*z for y, z in zip(close, volume)]), window=vwma_length)), list(sma_indicator(pd.Series(volume), window=vwma_length)))]

    avg = [(x + y) / 2 for x, y in zip(xATRTrailingStop, vwma)]

    vol_avg = list(sma_indicator(pd.Series(volume), window=vol_length))

    unusual_vol_down = [volume[i] > vol_avg[i] * vol_mult and close[i] < Open[i] for i in range(len(close))]
    unusual_vol_up = [volume[i] > vol_avg[i] * vol_mult and close[i] > Open[i] for i in range(len(close))]

    return pos, avg, unusual_vol_down, unusual_vol_up


def JL_ATR_and_Unusual_Volume(Trade_Direction, close, pos, avg, unusual_vol_down, unusual_vol_up, current_index):
    close_pos = 0

    if pos[current_index] == -1 and Trade_Direction != 1 and unusual_vol_up[current_index]:
        Trade_Direction = 0
    elif pos[current_index] == 1 and Trade_Direction != 0 and unusual_vol_down[current_index]:
        Trade_Direction = 1
    elif pos[current_index] == 0:
        Trade_Direction = -99

    if close[current_index] < avg[current_index] and Trade_Direction == 1:
        close_pos = 1
    elif close[current_index] > avg[current_index] and Trade_Direction == 0:
        close_pos = 1

    return Trade_Direction, close_pos, 99999, 99999
